...
&gt; **Claimed an issuer subdomain?** If your organization has claimed an [issuer subdomain](/docs/domains/#issuer-subdomains), your tokens are issued under that URL (e.g. `https://acme-com.us.vouch.sh`) instead of the shared instance URL. Use the issuer and discovery URLs shown on `/admin/subdomain` for the provider URL, the client ID, and the trust policy conditions below.
...
- Verify the OIDC provider URL in the IAM trust policy matches `https://us.vouch.sh` exactly (no trailing slash) — or your [issuer subdomain](/docs/domains/#issuer-subdomains) URL if your organization has claimed one.
...
- Ensure the OIDC provider in AWS points to the correct Vouch server URL: `https://us.vouch.sh`, or your claimed issuer subdomain URL.
