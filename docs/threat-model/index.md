...
- **T-S4**: SCIM tokens are stored as SHA-256 hashes — a database read cannot recover them. Domain validation restricts user creation to the organization&#39;s verified domains, and every provisioning operation is audited (`scim_operation`). Tokens are revocable at any time from `/admin/scim-tokens`, and audit-trail read access must be explicitly granted when a token is minted. → [SCIM Provisioning](/docs/scim/)
