...
Yes. All communication between the CLI and server uses TLS 1.3, with TLS 1.2 accepted using modern AEAD ciphers only. See [Security](/docs/security/#encryption) for details.
...
Yes. Vouch connects to the Vouch server over HTTPS (port 443). As long as your VPN allows outbound HTTPS traffic to `us.vouch.sh`, Vouch works normally.
...
Vouch requires outbound HTTPS (port 443) to the Vouch server (`us.vouch.sh`). No inbound ports are required. If your firewall performs TLS inspection, ensure it does not interfere with the FIDO2 WebAuthn flow during enrollment and login.
