...
- **Registration** (enrollment): The YubiKey generates a key pair. The public key is sent to the server along with an attestation certificate. The private key never leaves the hardware. The server validates the attestation certificate chain against pinned Yubico root CA certificates to confirm the key is a genuine hardware device; a registration without a verifiable chain is rejected. If the account already has a registered key, the server requires an assertion with it before accepting a new registration.
