...
> Privacy policy for the Vouch authentication service.

Source: https://vouch.sh/privacy/
...
- **Blind equality indexes** -- Database lookups (e.g., by email address) use HMAC-based blind indexes, so the database never stores plaintext identifiers.
...
Vouch operates independent regional instances (for example, the United States, European Union, Japan, and Asia-Pacific). Each instance is fully isolated, and no data is shared between instances. Your organization&#39;s data is stored and processed within the region of the Vouch instance your organization uses. If you access the Service from outside that region, your information will be transferred to, stored, and processed in that region. By using the Service, you consent to this transfer.
