...
```
YubiKey tap → FIDO2 → Vouch JWT → STS → Bedrock InvokeModel → CloudTrail
```
...
- **Cost attribution per user** -- with [Bedrock model invocation logging](https://docs.aws.amazon.com/bedrock/latest/userguide/model-invocation-logging.html) enabled, token counts and costs are tied to hardware-verified identities.

---

## Step 1 -- Use Bedrock with Vouch

Any tool that uses the AWS SDK for Bedrock will pick up Vouch credentials automatically:
...
CloudTrail records every Bedrock API call with the full identity chain. The `webIdFederationData` field includes the Vouch OIDC issuer and the user's email (from the `sub` claim).

With [Bedrock model invocation logging](https://docs.aws.amazon.com/bedrock/latest/userguide/model-invocation-logging.html) enabled, you get token counts and costs attributed to hardware-verified identities.

---

## Agent delegation

For automated agents that call Bedrock on behalf of users, you can use scoped JWTs with agent-specific `sub` claims and session policies that limit model access. This preserves the human identity chain while restricting the agent's capabilities to only the models it needs.
